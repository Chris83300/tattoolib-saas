...
class TattooerTraceabilityController extends ArtisanBaseController
{
    ...
    public function storeClientTraceability(Request $request, Client $client)
    {
        ...
        $validated = $request->validate([
            ...
            'tattoo_description'     => 'nullable|string|max:500',
            'body_zone'              => 'nullable|string|max:100',
            'procedure_start_time'   => 'nullable|date_format:H:i',
            'procedure_end_time'     => 'nullable|date_format:H:i',
            ...
            // Photos de lots (même champ que storeTraceability)
            'needles'                => 'nullable|array',
            'needles.*'              => 'nullable|array',
            'needles.*.brand'        => 'nullable|string|max:255',
            'needles.*.lot_number'   => 'nullable|string|max:255',
        ...
        // Filtrer les encres vides
        $inks = $validated['inks'] ?? [];
        $inks = array_values(array_filter($inks, fn($i) =>
            !empty(trim($i['brand'] ?? '')) || !empty(trim($i['color'] ?? '')) || !empty(trim($i['lot_number'] ?? ''))
        ));
        ...
        $traceability = TraceabilityRecord::create([
            ...
            'tattoo_description' => $validated['tattoo_description'] ?? null,
            'body_zone'      => $validated['body_zone'] ?? null,
            'procedure_date' => $validated['session_date'],
            'procedure_start_time' => $validated['procedure_start_time'] ?? null,
            'procedure_end_time'   => $validated['procedure_end_time'] ?? null,
            ...
            'aftercare_products'     => $validated['aftercare_products'] ?? null,
            'room_number'            => $validated['room_number'] ?? null,
            'autoclave_batch_number' => $validated['autoclave_batch_number'] ?? null,
            'autoclave_test_date'    => $validated['autoclave_test_date'] ?? null,
            'procedure_notes'        => $validated['other_supplies'] ?? $validated['procedure_notes'] ?? null,
            'client_condition_notes' => $validated['client_condition_notes'] ?? null,
            'equipment_notes'        => $validated['notes'] ?? $validated['equipment_notes'] ?? null,
            'tattooer_verified_traceability' => true,
            'verified_at' => now(),
        ]);

        // Upload photos de lots
        if ($request->hasFile('lot_photos')) {
            foreach ($request->file('lot_photos') as $photo) {
                $traceability->addMedia($photo)->toMediaCollection('lot_photos');
            }
