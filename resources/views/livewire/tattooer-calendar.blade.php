        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Infos RDV -->
                    @if($selectedEvent->type === 'appointment' && $selectedEvent->project)
                        <div class="border-t pt-4">
                            <h4 class="font-medium text-gray-900 mb-2">Détails du rendez-vous</h4>
                            <div class="space-y-2 text-sm">
                                <div>
                                    <span class="text-gray-600">Client:</span>
                                    <span class="ml-2">{{ $selectedEvent->project->client->full_name }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Projet:</span>
                                    <span class="ml-2">{{ $selectedEvent->project->tattoo_description }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Emplacement:</span>
                                    <span class="ml-2">{{ $selectedEvent->project->tattoo_location }}</span>
                                </div>
                                @if($selectedEvent->project->deposit_amount)
                                    <div>
                                        <span class="text-gray-600">Acompte:</span>
                                        <span class="ml-2">{{ number_format($selectedEvent->project->deposit_amount, 2) }}€</span>
                                        @if($selectedEvent->project->isDepositPaid())
            ...
            <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                ...
                <form wire:submit="createEvent" class="space-y-4">
                    ...
                    <div>
                        ...
                        @error('eventType') <span class="text-red-500 text-sm">{{ $message }}</span> @endif
                    </div>
