                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-3">
                                ...
                                <div class="w-12 h-12 bg-beige-peau/20 rounded-full flex items-center justify-center">
                                    @if ($bookingRequest->bookable && $bookingRequest->bookable->user->getFirstMediaUrl('avatar'))
                                    ...
                                    @else
                                        <span class="text-beige-peau font-bold text-lg">
                                            {{ substr($bookingRequest->bookable->user->pseudo ?? 'Artiste', 0, 2) }}
                                        </span>
                                    @endif
                                </div>
                                <div>
                                    <h3 class="text-ivoire-text font-semibold">
                                        {{ $bookingRequest->bookable->user->pseudo ?? 'Artiste non assigné' }}
                                    </h3>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                                <div>
                                    ...
                                    <p class="text-ivoire-text font-medium">
                                        {{ $bookingRequest->created_at->format('d/m/Y') }}</p>
                                </div>
                                <div>
                                    <span class="text-ivoire-text/50 text-xs">RDV</span>
                                    <p class="text-ivoire-text font-medium">
                                        @if ($bookingRequest->appointment && $bookingRequest->appointment->appointment_datetime)
                                ...
                                <div>
                                    ...
                                    <span
                                        class="inline-block px-2 py-1 text-xs rounded-full font-semibold
                                                @switch($bookingRequest->status->value)
                                                    @case('pending')
                            ...
                            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-ivoire-text text-sm mt-1">
                                        {{ \Illuminate\Support\Str::limit($bookingRequest->description ?? 'Non spécifié', 80) }}
                                    </p>
                                </div>
                                <div>
                                    <span class="text-ivoire-text/50 text-xs">Localisation</span>
                                    <p class="text-ivoire-text text-sm mt-1">
                                        {{ $bookingRequest->body_zone ?? 'Non spécifiée' }}
                                    </p>
                                </div>
                            </div>
