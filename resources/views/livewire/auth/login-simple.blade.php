...
<div class="min-h-screen bg-noir-profond flex items-center justify-center px-4">
    <div class="max-w-md w-full">
        ...
        <form method="POST" action="{{ route('login.store') }}" class="bg-gris-fonde rounded-xl p-6 space-y-4">
            @csrf
        ...
        <div class="text-center mt-4">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-ivoire-text/50 text-sm hover:text-beige-peau hover:underline">
                    Mot de passe oublié ?
                </a>
            @endif
        </div>
