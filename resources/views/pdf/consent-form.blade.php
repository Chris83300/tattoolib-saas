...
@section('content')
    ...
    <div class="info-grid">
        <div class="info-col">
            ...
            <div class="info-value">
                {{ $consentForm->client_birth_date?->format('d/m/Y') ?? ($consentForm->birth_date?->format('d/m/Y') ?? '—') }}
            </div>
            <div class="info-label">Pièce d'identité</div>
            <div class="info-value">{{ $consentForm->client_id_type ?? ($consentForm->id_document_type ?? '—') }} —
                {{ $consentForm->client_id_number ?? ($consentForm->id_document_number ?? '—') }}</div>
        </div>
        <div class="info-col">
    ...
    @if ($consentForm->is_minor)
        <div class="alert-box">
            <strong>Client mineur :</strong> {{ $consentForm->parent_name ?? '—' }}
            ({{ $consentForm->parent_relation ?? '—' }}) — N° pièce d'identité : {{ $consentForm->parent_id_number ?? '—' }}
        </div>
    @endif
    ...
    <div class="info-grid">
        ...
        <div class="info-col">
            ...
            @if ($consentForm->total_price)
                ...
                <div class="info-value">{{ number_format($consentForm->total_price, 2, ',', ' ') }} €</div>
            @endif
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            <tr>
                ...
                @if ($consentForm->medical_allergies_detail)
                    <td class="text-small">{{ $consentForm->medical_allergies_detail }}</td>
                @endif
            </tr>
            <tr>
            ...
            <tr>
                ...
                @if ($consentForm->medical_skin_disease_detail)
                    <td class="text-small">{{ $consentForm->medical_skin_disease_detail }}</td>
                @endif
            </tr>
            <tr>
                <td>VIH / Hépatite</td>
    ...
    <div class="signature-block mt-20">
            ...
            <div class="signature-line">
                @if ($consentForm->signed_at)
                    <div class="text-small text-muted" style="padding-top: 5px;">Signé le
                        {{ $consentForm->signed_at->format('d/m/Y à H:i') }}</div>
                @endif
            </div>
        ...
        <div class="signature-col">
            ...
            <div class="signature-line">
                @if ($consentForm->signature_data)
                    <div style="margin-bottom: 10px;">
                        <img src="{{ $consentForm->signature_data }}" alt="Signature du client"
