        ...
        <nav class="hidden md:flex items-center gap-6">
            ...
            @auth
                ...
                @if(auth()->user()->role === 'client')
                    <a href="{{ route('client.bookings') }}" class="text-ivoire-text hover:text-beige-peau transition-colors">
                        Mes RDV
                    </a>
                    <a href="{{ route('client.messages') }}" class="text-ivoire-text hover:text-beige-peau transition-colors relative">
                        Messages
                        @if(auth()->user()->unread_messages_count > 0)
                            <span class="absolute -top-1 -right-1 bg-rouge-alerte text-noir-profond text-xs font-bold px-1.5 rounded-full">
                ...
                @if(in_array(auth()->user()->role, ['tattooer', 'pierceur', 'studio_artist']))
                    ...
                    <a href="{{ route('tattooer.booking-requests') }}" class="text-ivoire-text hover:text-beige-peau transition-colors">
                        Demandes
                    </a>
                @endif
