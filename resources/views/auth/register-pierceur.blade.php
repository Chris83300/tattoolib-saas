            ...
            <form action="{{ route('register.pierceur.submit') }}" method="POST"
                ...
                <div class="border-b border-titane/20 pb-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            @foreach ([
                                'pierceur' => ['label' => 'Pierceur', 'icon' => '💉'],
                                'bodemodeur' => ['label' => 'Body Modeur', 'icon' => '⚡'],
                                'pierceur_bodemodeur' => ['label' => 'Pierceur & Body Modeur', 'icon' => '💎'],
                            ] as $value => $opt)
                                <label class="flex items-center gap-3 p-3 bg-noir-profond rounded-lg cursor-pointer border border-titane/30 hover:border-beige-peau/50 transition-colors has-[:checked]:border-beige-peau has-[:checked]:bg-beige-peau/10">
                                    ...
                                    <span class="text-ivoire-text text-sm font-semibold">{{ $opt['icon'] }} {{ $opt['label'] }}</span>
                                </label>
                            @endforeach
                        </div>
                ...
                <div class="border-b border-titane/20 pb-6">
                    ...
                    <div class="space-y-4">
                        <!-- Prénom -->
                        <div>
                            <label class="block text-ivoire-text text-sm font-semibold mb-2">Prénom *</label>
                            <input type="text" name="first_name" required placeholder="Jean" value="{{ old('first_name') }}"
                            ...
                            @error('first_name') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <!-- Nom -->
                        <div>
                            <label class="block text-ivoire-text text-sm font-semibold mb-2">Nom *</label>
                            <input type="text" name="last_name" required placeholder="Dupont" value="{{ old('last_name') }}"
                                class="w-full bg-noir-profond text-ivoire-text px-4 py-3 rounded-lg border border-titane/30 focus:border-beige-peau focus:ring-2 focus:ring-beige-peau focus:ring-opacity-50 transition-colors">
                            @error('last_name') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="pseudo" placeholder="Ex: PierceurPro75" value="{{ old('pseudo') }}"
                                class="w-full bg-noir-profond text-ivoire-text px-4 py-3 rounded-lg border border-titane/30 focus:border-beige-peau focus:ring-2 focus:ring-beige-peau focus:ring-opacity-50 transition-colors">
                            <p class="text-ivoire-text/50 text-xs mt-1">Ce pseudo sera affiché sur votre profil public</p>
                            @error('pseudo') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <!-- Email -->
                        <div>
                            <label class="block text-ivoire-text text-sm font-semibold mb-2">Email professionnel *</label>
                            <input type="email" name="email" required value="{{ old('email') }}"
                                class="w-full bg-noir-profond text-ivoire-text px-4 py-3 rounded-lg border border-titane/30 focus:border-beige-peau focus:ring-2 focus:ring-beige-peau focus:ring-opacity-50 transition-colors">
                            @error('email') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <!-- Password -->
                        <div>
                            <label class="block text-ivoire-text text-sm font-semibold mb-2">
                                Mot de passe * <span class="text-ivoire-text/50 font-normal">(8 car. min, 1 majuscule, 1 chiffre, 1 spécial)</span>
                            </label>
                            <input type="password" name="password" required minlength="8" placeholder="•••••••••"
                                class="w-full bg-noir-profond text-ivoire-text px-4 py-3 rounded-lg border border-titane/30 focus:border-beige-peau focus:ring-2 focus:ring-beige-peau focus:ring-opacity-50 transition-colors">
                            @error('password') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <!-- Password confirmation -->
                        <div>
                            <label class="block text-ivoire-text text-sm font-semibold mb-2">Confirmer mot de passe *</label>
                            <input type="password" name="password_confirmation" required minlength="8" placeholder="•••••••••"
                                class="w-full bg-noir-profond text-ivoire-text px-4 py-3 rounded-lg border border-titane/30 focus:border-beige-peau focus:ring-2 focus:ring-beige-peau focus:ring-opacity-50 transition-colors">
                            @error('password_confirmation') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-ivoire-text text-sm font-semibold mb-2">Ville *</label>
                            <input type="text" name="city" required value="{{ old('city') }}"
                                class="w-full bg-noir-profond text-ivoire-text px-4 py-3 rounded-lg border border-titane/30 focus:border-beige-peau focus:ring-2 focus:ring-beige-peau focus:ring-opacity-50 transition-colors">
                            @error('city') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-ivoire-text text-sm font-semibold mb-2">Code postal *</label>
                            <input type="text" name="postal_code" required value="{{ old('postal_code') }}"
                                class="w-full bg-noir-profond text-ivoire-text px-4 py-3 rounded-lg border border-titane/30 focus:border-beige-peau focus:ring-2 focus:ring-beige-peau focus:ring-opacity-50 transition-colors">
                            @error('postal_code') <p class="text-rouge-alerte text-xs mt-1">{{ $message }}</p> @enderror
                        </div>
                    </div>

                    <div class="mt-4">
                        <label class="block text-ivoire-text text-sm font-semibold mb-2">Téléphone (optionnel)</label>
