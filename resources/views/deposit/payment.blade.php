            ...
            <div class="bg-[#D4B59E]/10 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <p class="font-medium">{{ $project->bookable->user->name }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="font-medium">
                            @if($project->appointment_date)
                                {{ $project->appointment_date->format('d/m/Y à H:i') }}
                            @else
                                À définir
                            @endif
                        </p>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 mb-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center py-3 border-b">
                    ...
                    <span class="font-semibold text-lg">{{ number_format($project->estimated_price, 2) }}€</span>
                </div>
                ...
                <div class="flex justify-between items-center py-3 border-b">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">
                            {{ round(($project->deposit_amount / $project->estimated_price) * 100, 1) }}% du total
                        </p>
                    </div>
                    <span class="font-semibold text-lg text-[#D4B59E]">{{ number_format($project->deposit_amount, 2) }}€</span>
                </div>
                ...
                <div class="flex justify-between items-center py-3">
                    ...
                    <span class="font-semibold text-lg">
                        {{ number_format($project->estimated_price - $project->deposit_amount, 2) }}€
                    </span>
                </div>
            </div>

            <!-- Images de référence -->
            @if($project->getMedia('reference_images')->count() > 0)
                <div class="mt-6">
                    <h3 class="text-sm font-medium text-gray-700 mb-2">Images de référence</h3>
                    <div class="grid grid-cols-3 gap-2">
                        @foreach($project->getMedia('reference_images')->take(3) as $media)
                            <img src="{{ $media->getUrl('thumbnail') }}" 
                                 alt="Reference"
                                 class="w-full h-20 object-cover rounded">
                        @endforeach
                        @if($project->getMedia('reference_images')->count() > 3)
                            <div class="w-full h-20 bg-gray-200 rounded flex items-center justify-center text-sm text-gray-600">
                                +{{ $project->getMedia('reference_images')->count() - 3 }}
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-lg p-6">
            <div class="text-center">
                ...
                <button id="checkout-button" 
                    ...
                    <span id="button-text">Payer l'acompte de {{ number_format($project->deposit_amount, 2) }}€</span>
                    ...
                    <div class="flex items-center justify-center space-x-2">
                        ...
                        <div class="flex items-center space-x-1 text-gray-400">
                            <svg class="w-8 h-5" viewBox="0 0 24 16" fill="currentColor">
                                <rect x="1" y="4" width="22" height="12" rx="2" stroke="currentColor" stroke-width="2" fill="none"/>
...
<script>
    ...
    checkoutButton.addEventListener('click', async () => {
        ...
        try {
            ...
            const response = await fetch('{{ route("deposit.checkout.session", $project->id) }}', {
                method: 'POST',
                headers: {
