                    ...
                    @forelse ($this->getConversations() as $conv)
                        ...
                        <button wire:click="selectConversation({{ $conv->id }})"
                            ...
                            <p class="text-sm font-medium text-gray-900 dark:text-white truncate">
                                {{ $conv->participants->map(fn($u) => $u->name ?? 'Utilisateur')->join(', ') }}
                            </p>
        ...
        <div class="lg:col-span-2">
            <x-filament::section class="h-full">
                @if ($conversation = $this->getActiveConversation())
                    ...
                    <div class="mb-4 p-2.5 rounded-lg bg-warning-50 dark:bg-warning-500/10 border border-warning-200 dark:border-warning-500/20">
                        ...
                        @foreach ($conversation->messages->sortBy('created_at') as $message)
                            ...
                            <div class="flex {{ $isArtist ? 'justify-end' : 'justify-start' }}">
                                <div class="max-w-[75%] p-3 rounded-xl text-sm
                                    ...
                                    <p class="text-[10px] font-semibold mb-1 {{ $isArtist ? 'text-primary-600 dark:text-primary-400' : 'text-gray-500' }}">
