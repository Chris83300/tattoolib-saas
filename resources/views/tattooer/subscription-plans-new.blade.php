...
<div class="max-w-4xl mx-auto space-y-6">
            ...
            @if ($activeSubscription?->isOnGracePeriod())
                <div class="mt-3 bg-ambre-warning/10 border border-ambre-warning/30 rounded-lg p-3">
                    ...
                    <form action="{{ route('tattooer.subscription.resume') }}" method="POST" class="mt-2">
                        @csrf
                        <button type="submit" class="px-4 py-2 bg-vert-succes text-white rounded-lg text-sm font-semibold hover:bg-vert-succes/90">
            ...
            @else
                <div class="mt-3 flex flex-wrap gap-3">
                    ...
                    <form action="{{ route('tattooer.subscription.cancel') }}" method="POST"
                        ...
                        @csrf
                        <button type="submit" class="px-4 py-2 border border-rouge-alerte/30 text-rouge-alerte rounded-lg text-sm hover:bg-rouge-alerte/10 transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {{-- Plan FREE --}}
        <div class="bg-gris-fonde rounded-xl p-6 border {{ $tattooer->isFree() ? 'border-titane/40' : 'border-titane/20' }}">
            <h3 class="text-xl font-bold text-ivoire-text mb-1">Free</h3>
            <p class="text-3xl font-bold text-ivoire-text mb-4">0€<span class="text-sm font-normal text-titane">/mois</span></p>
        ...
        <div class="bg-gris-fonde rounded-xl p-6 border-2 border-beige-peau relative">
            <div class="absolute -top-3 left-4 px-3 py-0.5 bg-beige-peau text-noir-profond text-xs font-bold rounded-full">
                RECOMMANDÉ
            </div>

            <h3 class="text-xl font-bold text-ivoire-text mb-1">PRO</h3>
            <p class="text-3xl font-bold text-beige-peau mb-4">49.99€<span class="text-sm font-normal text-titane">/mois</span></p>
            ...
            <ul class="space-y-2 text-sm text-ivoire-text/80 mb-6">
                ...
                <li class="flex items-center gap-2 text-vert-succes">✅ Fiche client manuelle</li>
                ...
                <li class="flex items-center gap-2 text-vert-succes">✅ Branding personnalisé</li>
            </ul>
    ...
    {{-- ROI Calculator --}}
    @if ($tattooer->isFree())
    <div class="bg-gris-fonde rounded-xl p-6 border border-titane/20" x-data="{ monthly: 2000 }">
        <h3 class="text-lg font-bold text-ivoire-text mb-3">💡 Calculez votre économie PRO</h3>
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div class="flex-1">
                <label class="text-sm text-titane block mb-1">Revenus mensuels estimés (€)</label>
                <input type="range" min="500" max="10000" step="100" x-model="monthly"
                       class="w-full accent-beige-peau">
                <p class="text-sm text-ivoire-text mt-1"><span x-text="Number(monthly).toLocaleString('fr-FR')"></span>€/mois</p>
            </div>
            <div class="text-center bg-noir-profond rounded-xl p-4 min-w-[150px]">
                <p class="text-xs text-titane">Économie mensuelle</p>
                <p class="text-2xl font-bold text-vert-succes" x-text="((monthly * 0.07) - 49.99).toFixed(0) + '€'"></p>
                <p class="text-xs text-titane mt-1" x-text="monthly * 0.07 > 49.99 ? '✅ PRO rentable !' : '⚠️ Free plus avantageux'"></p>
            </div>
        </div>
    </div>
    @endif
