            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-2">
                        ...
                        {{-- Suppression : uniquement les fiches créées manuellement --}}
                        @if ($client->tattooer_id === $tattooer->id)
                            <form action="{{ route($tattooer->routePrefix() . '.client.destroy', $client) }}"
                                  method="POST"
                                  x-data
                                  @submit.prevent="if(confirm('Supprimer définitivement cette fiche client ? Cette action est irréversible.')) $el.submit()">
                                @csrf @method('DELETE')
                                <button type="submit"
                                    class="inline-flex items-center gap-1 px-3 py-1.5 bg-rouge-alerte/10 text-rouge-alerte border border-rouge-alerte/30 rounded-lg text-xs font-semibold hover:bg-rouge-alerte/20 transition-colors">
        ...
        <div x-show="activeTab === 'info'" x-cloak class="space-y-4">
            ...
            <div class="bg-gris-fonde rounded-xl p-4">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route($tattooer->routePrefix() . '.messages') }}"
            ...
            @forelse ($bookingRequests as $br)
                <div class="bg-gris-fonde rounded-xl p-4">
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <a href="{{ route($tattooer->routePrefix() . '.request.show', $br) }}"
                            class="p-2 bg-noir-profond rounded-lg hover:bg-beige-peau/20 transition-colors flex-shrink-0">
                            <svg class="w-4 h-4 text-beige-peau" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <x-pro-gate feature="la gestion des consentements SNAT">
                ...
                <div class="bg-gris-fonde rounded-xl p-4 md:p-6" x-data="{ consentMode: 'none' }"
                    ...
                    <div x-show="consentMode === 'upload'" x-cloak x-collapse>
                        <form action="{{ route($tattooer->routePrefix() . '.clients.consent.upload', $client) }}"
                    ...
                    <div x-show="consentMode === 'digital'" x-cloak x-collapse>
                        <form action="{{ route($tattooer->routePrefix() . '.clients.consent.store-digital', $client) }}"
                                        ...
                                        <div x-data="{ checked: false }">
                                            ...
                                            @if ($config['detail'])
                                                <div x-show="checked" x-cloak x-collapse class="mt-1 ml-6">
                                                    <input type="text" name="{{ $field }}_detail"
                                    ...
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                            ...
                                            <p><span class="text-ivoire-text/60">Pièce:</span>
                                                {{ $consent->client_id_type ? ucfirst($consent->client_id_type) : 'N/R' }}
                                                - {{ $consent->client_id_number ?? 'N/R' }}</p>
                                        </div>
                                    </div>
                                    @if ($consent->is_minor)
                                        <div class="bg-ambre-warning/10 border border-ambre-warning/30 rounded-lg p-3">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                                <p><span class="text-ivoire-text/60">Parent:</span>
                                                    {{ $consent->parent_name ?? 'N/R' }}</p>
                                                <p><span class="text-ivoire-text/60">Relation:</span>
                                                    {{ $consent->parent_relation ? ucfirst($consent->parent_relation) : 'N/R' }}
                                                </p>
                                                <p><span class="text-ivoire-text/60">N° pièce:</span>
                                                    {{ $consent->parent_id_number ?? 'N/R' }}</p>
                                            </div>
                                        </div>
                                    @endif
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="space-y-1 text-sm">
                                            @if ($consent->medical_allergies)
                                                <p>🤧 Allergies: {{ $consent->medical_allergies_detail ?: 'Oui' }}</p>
                                            @endif
                                            ...
                                            @if ($consent->medical_skin_disease)
                                                <p>🩹 Maladie peau: {{ $consent->medical_skin_disease_detail ?: 'Oui' }}
                                                </p>
                                            @endif
                                            ...
                                            @if ($consent->medical_other)
                                                <p>📝 Autres: {{ $consent->medical_other }}</p>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                                            <p><span class="text-ivoire-text/60">Prix:</span>
                                                {{ $consent->total_price ? number_format($consent->total_price, 2, ',', ' ') . ' €' : 'N/R' }}
                                            </p>
                                            <p><span class="text-ivoire-text/60">Acompte:</span>
                                                {{ $consent->deposit_amount ? number_format($consent->deposit_amount, 2, ',', ' ') . ' €' : 'N/R' }}
                                            </p>
                                    ...
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="space-y-1 text-sm">
                                            <p><span class="text-ivoire-text/60">Mention:</span>
                                                {{ $consent->handwritten_mention ?? 'N/R' }}</p>
            ...
            <x-pro-gate feature="la traçabilité réglementaire">
                ...
                <div class="bg-gris-fonde text-ivoire-text rounded-xl p-4" x-data="{
                    ...
                    <div x-show="showForm" x-cloak x-collapse class="mt-4 pt-4 border-t border-titane/20">
                        <form action="{{ route($tattooer->routePrefix() . '.clients.traceability.store', $client) }}"
                                    ...
                                    <div class="flex items-start justify-between mb-2">
                                        <div>
                                            <p class="font-semibold text-ivoire-text">
                                                {{ $trace->tattoo_description ?? 'Tatouage' }}</p>
                                            <p class="text-sm text-titane">{{ $trace->body_zone ?? '' }} ·
                                                {{ $trace->session_date?->format('d/m/Y') ?? '' }}</p>
                                        </div>
                                        <div class="flex items-center gap-2">
                ...
                @forelse ($relevantAppointments as $apt)
                    ...
                    <div class="bg-gris-fonde rounded-xl p-4" x-data="{
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-titane">RDV
                                        {{ $apt->start_datetime?->translatedFormat('l d F Y à H:i') ?? 'Date à définir' }}
                                    </p>
                                </div>
                            </div>
                            <svg class="w-5 h-5 text-titane transition-transform" :class="expanded ? 'rotate-180' : ''"
                        ...
                        <div x-show="expanded" x-cloak x-collapse class="mt-4 pt-4 border-t border-titane/20">
                            <form action="{{ route($tattooer->routePrefix() . '.traceability.store', $apt) }}"
                                method="POST" enctype="multipart/form-data" class="space-y-4">
                                @csrf
        ...
        <div x-show="activeTab === 'media'" x-cloak class="space-y-4">
            <x-pro-gate feature="la galerie médias client">
                ...
                <div class="bg-gris-fonde rounded-xl p-4 md:p-6">
                    ...
                    <form action="{{ route($tattooer->routePrefix() . '.clients.photos.upload', $client) }}"
                ...
                @foreach ($bookingRequests->filter(fn($br) => $br->deposit_paid_at) as $br)
                    <div class="bg-gris-fonde rounded-xl p-4">
                        <h4 class="text-sm font-bold text-ivoire-text mb-3">📸 {{ $br->tattoo_style ?? 'Tattoo' }} —
                            {{ $br->body_zone ?? '' }} <span
                        ...
                        @endif
                        <form action="{{ route($tattooer->routePrefix() . '.client.photos.upload', [$client, $br]) }}"
                            method="POST" enctype="multipart/form-data"
        ...
        <div x-show="activeTab === 'notes'" x-cloak>
            <x-pro-gate feature="les notes privées client">
                <div class="bg-gris-fonde rounded-xl p-4 md:p-6">
                    ...
                    <form action="{{ route($tattooer->routePrefix() . '.client.update-notes', $client) }}"
                        method="POST">
                        @csrf
