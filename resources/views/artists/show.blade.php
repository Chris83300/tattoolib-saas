...
@section('title', $artist->user->name . ' - Artiste ' . ucfirst($artist->type ?? 'Tatoueur'))

@section('meta')
    <meta name="description"
        content="Découvrez {{ $artist->user->name }}, artiste {{ $artist->type ?? 'tatoueur' }} à {{ $artist->city }}. Portfolio, avis clients et réservation en ligne.">
    <meta property="og:title" content="{{ $artist->user->name }} - Ink&Pik">
    <meta property="og:image" content="{{ $artist->getFirstMediaUrl('avatar') }}">
@endsection
...
@section('content')
    <div class="min-h-screen bg-noir-profond pb-20">
        ...
        <div
            class="sticky top-0 z-40 bg-noir-profond/95 backdrop-blur-sm border-b border-titane/20 md:hidden {{ request()->get('preview') === 'true' ? 'mt-12' : '' }}">
            <div class="container mx-auto px-4 h-14 flex items-center justify-between">
                ...
                <span class="text-ivoire-text font-semibold">{{ $artist->user->displayName() }}</span>

                <button x-data
                    @click="navigator.share({title: '{{ $artist->user->displayName() }}', url: window.location.href})"
        ...
        <div class="container mx-auto px-4 py-8 max-w-6xl">
            ...
            <div class="text-center mb-8">
                ...
                <div
                    ...
                    @if ($artist->hasMedia('avatar'))
                        <img src="{{ $artist->getFirstMediaUrl('avatar', 'thumb') }}"
                            alt="{{ $artist->user->displayName() }}" class="w-full h-full object-cover">
                    @else
                ...
                @if ($artist->specialties && is_array($artist->specialties))
            ...
            <div class="grid grid-cols-3 gap-4 mb-8">
                ...
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    ...
                    <p class="text-2xl font-bold text-beige-peau mb-1">{{ $stats['average_delay'] }}</p>
                ...
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    <div class="text-2xl mb-1">💰</div>
                    <p class="text-2xl font-bold text-beige-peau mb-1">{{ $stats['min_price'] }}€</p>
                ...
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    <div class="text-2xl mb-1">⭐</div>
                    <p class="text-2xl font-bold text-beige-peau mb-1">{{ number_format($stats['rating'], 1) }}/5</p>
            ...
            @if ($stats['platform_tattoos'] >= 15)
                <div class="bg-beige-peau/10 border border-beige-peau/30 rounded-xl p-4 mb-8 text-center">
                    ...
                    <p class="text-3xl font-bold text-beige-peau">{{ $stats['platform_tattoos'] }}</p>
                </div>
            @else
                <div class="bg-beige-peau/10 border border-beige-peau/30 rounded-xl p-4 mb-8 text-center">
                    ...
                    @if ($artist->address)
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-ivoire-text">{{ $artist->address }}</p>
                                <p class="text-ivoire-text">{{ $artist->city }}, {{ $artist->postal_code }}</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if ($artist->phone)
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <a href="tel:{{ $artist->phone }}" class="text-ivoire-text hover:text-beige-peau">
                                    {{ $artist->phone }}
                                </a>
                            </div>
                        </div>
            ...
            <div class="bg-gris-fonde rounded-xl p-6 mb-8" x-data="{ tab: 'realizations' }">
                ...
                <div class="flex gap-2 mb-6 overflow-x-auto pb-2">
                    <button @click="tab = 'realizations'"
                        ...
                        Réalisations ({{ $portfolioRealizations->count() }})
                    </button>
                    ...
                    <button @click="tab = 'drawings'"
                        :class="tab === 'drawings' ? 'bg-beige-peau text-noir-profond' :
                            'bg-noir-profond text-ivoire-text hover:bg-beige-peau/10'"
                        class="px-4 py-2 rounded-lg font-semibold text-sm whitespace-nowrap transition-colors">
                        Dessins ({{ $portfolioDrawings->count() }})
                    </button>
                    ...
                    <button @click="tab = 'beforeafter'"
                        :class="tab === 'beforeafter' ? 'bg-beige-peau text-noir-profond' :
                            'bg-noir-profond text-ivoire-text hover:bg-beige-peau/10'"
                        class="px-4 py-2 rounded-lg font-semibold text-sm whitespace-nowrap transition-colors">
                        Avant/Après Soins ({{ $portfolioBeforeAfter->count() / 2 }})
                    </button>
                </div>

                <!-- TAB 1 : Réalisations -->
                <div x-show="tab === 'realizations'">
                    @if ($portfolioRealizations->isNotEmpty())
                ...
                <div x-show="tab === 'drawings'" x-cloak>
                    @if ($portfolioDrawings->isNotEmpty())
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4" x-data="{ lightbox: false, currentImage: '' }">
                ...
                <div x-show="tab === 'beforeafter'" x-cloak>
                    @if ($portfolioBeforeAfter->isNotEmpty())
                        <div class="space-y-8">
                                ...
                                @if ($pair->count() === 2)
                                    <div class="max-w-2xl mx-auto">
                                        ...
                                        <p class="text-ivoire-text/70 text-sm mb-2 text-center">
                                            {{ $pair[0]->getCustomProperty('title') ?? 'Avant / Après cicatrisation' }}
                                        </p>

                                        <!-- Web Component img-comparison-slider -->
                                        <img-comparison-slider class="rounded-lg overflow-hidden">
                                            <img slot="first" src="{{ $pair[0]->getUrl() }}" alt="Avant" />
                                            <img slot="second" src="{{ $pair[1]->getUrl() }}" alt="Après" />
            ...
            <div
                ...
                <h3 class="text-2xl font-Satoshi font-bold text-ivoire-text mb-3">
                    Prêt à réserver avec {{ $artist->user->displayName() }} ?
                </h3>
