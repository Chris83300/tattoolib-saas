...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        @php
            ...
            $statusColors = [
                ...
                'deposit_paid'          => 'bg-vert-validation/20 text-vert-validation',
                'date_confirmed'        => 'bg-vert-validation/20 text-vert-validation',
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-gris-fonde rounded-xl p-5 space-y-4">
                ...
                @if($bookingRequest->description)
                    <div>
                        ...
                        <p class="text-sm text-ivoire-text/90 whitespace-pre-wrap">{{ $bookingRequest->description }}</p>
                    </div>
                @endif
                    ...
                    <div>
                        ...
                        <p class="text-sm text-ivoire-text">
                            @if($bookingRequest->confirmed_date)
                                <span class="text-vert-validation font-semibold">Confirmée : {{ \Carbon\Carbon::parse($bookingRequest->confirmed_date)->format('d/m/Y') }}</span>
                            @elseif($bookingRequest->preferred_date)
                                Souhaitée : {{ \Carbon\Carbon::parse($bookingRequest->preferred_date)->format('d/m/Y') }}
                            @endif
                        </p>
                    </div>
                @endif
            ...
            <!-- Financier -->
            @if($bookingRequest->deposit_amount || $bookingRequest->estimated_total_price || $bookingRequest->total_price)
                <div class="bg-gris-fonde rounded-xl p-5 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        @if($bookingRequest->estimated_total_price || $bookingRequest->total_price)
                            <div>
                                <p class="text-xs text-titane uppercase tracking-wide mb-1">Prix total</p>
                                <p class="text-lg font-bold text-beige-peau">
                                    {{ number_format(($bookingRequest->total_price ?? $bookingRequest->estimated_total_price) / 100, 2) }}€
                                </p>
                            </div>
                        @endif
                        @if($bookingRequest->deposit_amount)
                            <div>
                                ...
                                <p class="text-lg font-bold text-ivoire-text">
                                    {{ number_format($bookingRequest->deposit_amount / 100, 2) }}€
                                    @if($bookingRequest->deposit_paid_at)
                                        <span class="text-xs text-vert-validation font-normal ml-1">Payé</span>
                                    @endif
                                </p>
                            </div>
                        @endif
                    ...
                    @foreach($bookingRequest->messages->take(5) as $message)
                        <div class="p-4">
                            <div class="flex items-center gap-2 mb-1">
                                <span class="text-xs font-semibold text-beige-peau">{{ $message->sender?->name ?? 'Système' }}</span>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-gris-fonde rounded-xl p-5 space-y-3">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-ivoire-text">{{ $bookingRequest->bookable?->user?->name ?? 'Artiste' }}</p>
