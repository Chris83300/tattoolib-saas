    ...
    <div class="min-h-screen bg-noir-profond">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="mb-8">
                ...
                <p class="text-ivoire-text/70">
                    Bienvenue {{ Auth::user()->name }} ! Gérez vos demandes de tatouage.
                </p>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-beige-peau mb-1">{{ $stats['total_requests'] }}</div>
                    <div class="text-ivoire-text/60 text-xs">Total demandes</div>
                </div>
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-ambre-warning mb-1">{{ $stats['pending'] }}</div>
                    <div class="text-ivoire-text/60 text-xs">En attente</div>
                </div>
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-vert-succes mb-1">{{ $stats['accepted'] }}</div>
                    <div class="text-ivoire-text/60 text-xs">Acceptées</div>
                </div>
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-beige-peau mb-1">{{ $stats['active'] }}</div>
                    <div class="text-ivoire-text/60 text-xs">En cours</div>
                </div>
                <div class="bg-gris-fonde rounded-xl p-4 text-center">
                    <div class="text-2xl font-bold text-ivoire-text/50 mb-1">{{ $stats['completed'] }}</div>
                </div>
        ...
        <div class="bg-gris-fonde rounded-xl p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('client.booking-requests') }}" class="text-beige-peau hover:text-beige-peau/80 text-sm">
                    Voir tout →
                </a>
            </div>

            @if ($recentBookingRequests->isEmpty())
            ...
            @else
                <div class="space-y-4">
                    @foreach ($recentBookingRequests as $bookingRequest)
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        ...
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium"
                                            style="background-color: {{ $bookingRequest->status->color() }}15; color: {{ $bookingRequest->status->color() }}; border: 1px solid {{ $bookingRequest->status->color() }}30;">
                                    ...
                                    <div class="flex items-center gap-4 text-sm text-ivoire-text/60 mb-2">
                                        ...
                                        @if ($bookingRequest->estimated_price)
                                            <span>💰 {{ number_format($bookingRequest->estimated_price, 0) }}€</span>
                                        @endif
                                    </div>

                                    <div class="flex items-center gap-2 text-xs text-ivoire-text/50">
                                        <span>Artiste:
                                            {{ $bookingRequest->bookable->user->name ?? 'Non assigné' }}</span>
                                        <span>•</span>
                                        <span>{{ $bookingRequest->created_at->format('d/m/Y') }}</span>
                                    </div>
                                </div>
