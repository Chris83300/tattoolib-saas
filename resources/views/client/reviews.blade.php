        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                <div class="bg-titane/20 rounded-xl p-6 border border-titane/30 text-center">
                    <div class="text-2xl font-bold text-beige-peau mb-1">{{ $reviews->count() }}</div>
                        ...
                        <div class="bg-titane/20 rounded-xl p-6 border border-titane/30">
                            <div class="flex items-start justify-between mb-4">
                                ...
                                <div class="flex items-center gap-1">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <span class="text-{{ $i <= $review->rating ? 'beige-peau' : 'ivoire-text/30' }}">
                                            ⭐
                                        </span>
                                    @endfor
                                </div>
                            ...
                            @if ($review->tattooer_response)
                                <div class="bg-noir-profond rounded-lg p-4 border border-titane/30">
                                    ...
                                    <p class="text-ivoire-text/70 text-sm">{{ $review->tattooer_response }}</p>
                                </div>
                            @endif
                            <div class="text-ivoire-text/50 text-xs">
                                Avis laissé le {{ $review->created_at->format('d/m/Y à H:i') }}
                            </div>
                        </div>
                    @endforeach
                @endif
