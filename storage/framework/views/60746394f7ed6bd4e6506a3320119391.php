                ...
                <?php
                    ...
                    $tabs = [
                        ...
                        'trace' => [
                            'label' => 'Traçabilité',
                            'icon' => '🔬',
                            'count' => $traceabilities->count() + $standaloneTraces->count(),
                        ],
                        'media' => [
                            'label' => 'Médias',
                            'icon' => '📸',
                            'count' => $chatMedia->count() + $clientPhotos->count(),
                        ],
        ...
        <div x-show="activeTab === 'info'" x-cloak class="space-y-4">
            ...
            <div class="bg-gris-fonde rounded-xl p-4">
                <div class="flex flex-wrap gap-2">
                    <a href="<?php echo e(route($tattooer->routePrefix() . '.messages')); ?>"
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $bookingRequests; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $br): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="bg-gris-fonde rounded-xl p-4">
                    <div class="flex items-start justify-between mb-3">
                        ...
                        <a href="<?php echo e(route($tattooer->routePrefix() . '.request.show', $br)); ?>"
                            class="p-2 bg-noir-profond rounded-lg hover:bg-beige-peau/20 transition-colors flex-shrink-0">
                            <svg class="w-4 h-4 text-beige-peau" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<?php $component->withAttributes(['feature' => 'la gestion des consentements SNAT']); ?>
                ...
                <div class="bg-gris-fonde rounded-xl p-4 md:p-6" x-data="{ consentMode: 'none' }">
                    ...
                    <div x-show="consentMode !== 'none'" class="mb-4">
                        <button @click="consentMode = 'none'" type="button"
                            class="text-xs text-titane hover:text-beige-peau transition-colors">← Retour au choix</button>
                    </div>

                    
                    <div x-show="consentMode === 'upload'" x-cloak x-collapse>
                        <form action="<?php echo e(route($tattooer->routePrefix() . '.clients.consent.upload', $client)); ?>"
                            method="POST" enctype="multipart/form-data" class="space-y-3">
                            <?php echo csrf_field(); ?>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-semibold text-ivoire-text/60 mb-1">Fichier
                                        (PDF/JPG/PNG)</label>
                                ...
                                <div>
                                    <label class="block text-xs font-semibold text-ivoire-text/60 mb-1">Date du
                                        consentement</label>
                            ...
                            <button type="submit"
                                class="w-full sm:w-auto px-4 py-2 bg-beige-peau text-noir-profond rounded-lg text-sm font-semibold hover:bg-beige-peau/90 transition-colors">📤
                                Uploader</button>
                        </form>
                    </div>
                    ...
                    <div x-show="consentMode === 'digital'" x-cloak x-collapse>
                        <form action="<?php echo e(route($tattooer->routePrefix() . '.clients.consent.store-digital', $client)); ?>"
                            method="POST" class="space-y-4" id="digital-consent-form">
                            <?php echo csrf_field(); ?>
                            ...
                            <div class="bg-noir-profond/50 rounded-lg p-3" x-data="{ isMinor: false }">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="is_minor" value="1" x-model="isMinor"
                                        class="w-4 h-4 rounded border-titane/30 bg-noir-profond text-beige-peau focus:ring-beige-peau">
                            ...
                            <div class="bg-noir-profond/50 rounded-lg p-3">
                                <p class="text-xs font-bold text-ivoire-text/60 uppercase mb-3">🏥 Questionnaire médical
                                </p>
                                <div class="space-y-2">
                                    <?php
                                        $medicalFields = [
                                            'medical_allergies' => [
                                                'label' => '🤧 Allergies (latex, encres, pansements...)',
                                                'detail' => true,
                                            ],
                                            'medical_anticoagulant' => [
                                                'label' => '💉 Traitement anticoagulant',
                                                'detail' => false,
                                            ],
                                            'medical_diabetes' => ['label' => '🩸 Diabète', 'detail' => false],
                                            'medical_cicatrisation' => [
                                                'label' => '🩹 Problèmes de cicatrisation',
                                                'detail' => false,
                                            ],
                                            'medical_skin_disease' => [
                                                'label' => '🩹 Maladie de peau (eczéma, psoriasis...)',
                                                'detail' => true,
                                            ],
                                            'medical_vih_hepatite' => [
                                                'label' => '🔬 VIH / Hépatite',
                                                'detail' => false,
                                            ],
                                            'medical_pregnant' => [
                                            ...
                                            'medical_roaccutane' => [
                                                'label' => '💊 Traitement Roaccutane (< 1 an)',
                                                'detail' => false,
                                            ],
                                            'medical_cheloide' => [
                                                'label' => '⚕️ Tendance aux chéloïdes',
                                                'detail' => false,
                                            ],
                                        ];
                                    ?>
                                        ...
                                        <div x-data="{ checked: false }">
                                            ...
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($config['detail']): ?>
                                                <div x-show="checked" x-cloak x-collapse class="mt-1 ml-6">
                                                    <input type="text" name="<?php echo e($field); ?>_detail"
                                    ...
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    <div class="mt-2">
                                        <label class="text-xs text-titane block mb-1">📝 Autres pathologies ou
                                            traitements</label>
                            ...
                            <div class="bg-noir-profond/50 rounded-lg p-3">
                                <p class="text-xs font-bold text-ivoire-text/60 uppercase mb-3">🎨 Informations tatouage
                                </p>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="tattoo_description"
                                            placeholder="Ex : Mandala géométrique"
                                            class="w-full px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    ...
                                    <div class="flex items-end">
                                        <label class="flex items-center gap-2 cursor-pointer pb-2">
                                            <input type="checkbox" name="retouche_included" value="1"
                                                class="w-4 h-4 rounded border-titane/30 bg-noir-profond text-beige-peau focus:ring-beige-peau">
                                ...
                                <div class="flex flex-wrap gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer">
                                        <input type="radio" name="image_authorization" value="1"
                                            class="w-4 h-4 border-titane/30 bg-noir-profond text-beige-peau focus:ring-beige-peau">
                            ...
                            <div class="bg-noir-profond/50 rounded-lg p-3">
                                <p class="text-xs font-bold text-ivoire-text/60 uppercase mb-3">✅ Confirmations
                                    obligatoires</p>
                                <div class="space-y-2">
                                    <?php
                                        $confirmFields = [
                                            'confirm_medical_sincere' =>
                                                'Je déclare que les informations médicales sont sincères et complètes',
                                            'confirm_risks_informed' =>
                                                'J\'ai été informé(e) des risques liés au tatouage',
                                            'confirm_info_sheet_read' =>
                                                'J\'ai lu et compris la fiche d\'information pré-acte',
                                            'confirm_aftercare_received' =>
                                                'J\'ai reçu les consignes de soins post-tatouage',
                                            'confirm_not_intoxicated' =>
                                                'Je certifie ne pas être sous l\'emprise d\'alcool ou de stupéfiants',
                                            'confirm_over_18_or_authorized' =>
                                                'Je certifie être majeur(e) ou avoir l\'autorisation parentale',
                                    ...
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $confirmFields; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $field => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <label class="flex items-start gap-2 cursor-pointer">
                                            <input type="checkbox" name="<?php echo e($field); ?>" value="1" required
                                                class="w-4 h-4 rounded border-titane/30 bg-noir-profond text-beige-peau focus:ring-beige-peau mt-0.5">
                                            <span
                                                class="text-sm text-ivoire-text leading-tight"><?php echo e($label); ?></span>
                                        </label>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </div>
                            </div>
                            ...
                            <div class="bg-noir-profond/50 rounded-lg p-3">
                                ...
                                <div class="mb-3">
                                    <label class="text-xs text-titane block mb-1">Mention manuscrite : « Lu et approuvé »
                                        *</label>
                                    <input type="text" name="handwritten_mention" required
                                        placeholder="Écrire : Lu et approuvé"
                                        class="w-full px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text text-sm focus:border-beige-peau">
                                </div>
                                <div class="mb-2">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $consentDocuments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $document): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="flex items-center justify-between p-3 bg-noir-profond/50 rounded-lg">
                                    ...
                                    <div class="flex items-center gap-2 flex-shrink-0">
                                        ...
                                        <form
                                            action="<?php echo e(route($tattooer->routePrefix() . '.clients.consent.delete', [$client, $document->id])); ?>"
                                ...
                                <div class="space-y-3 text-ivoire-text">
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                            ...
                                            <p><span class="text-ivoire-text/60">Pièce:</span>
                                                <?php echo e($consent->client_id_type ? ucfirst($consent->client_id_type) : 'N/R'); ?>

                                                - <?php echo e($consent->client_id_number ?? 'N/R'); ?></p>
                                        </div>
                                    </div>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($consent->is_minor): ?>
                                        <div class="bg-ambre-warning/10 border border-ambre-warning/30 rounded-lg p-3">
                                            <p class="text-xs font-bold text-ambre-warning uppercase mb-2">👶 Autorisation
                                                parentale</p>
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                                                <p><span class="text-ivoire-text/60">Parent:</span>
                                                    <?php echo e($consent->parent_name ?? 'N/R'); ?></p>
                                                <p><span class="text-ivoire-text/60">Relation:</span>
                                                    <?php echo e($consent->parent_relation ? ucfirst($consent->parent_relation) : 'N/R'); ?>

                                                </p>
                                                <p><span class="text-ivoire-text/60">N° pièce:</span>
                                                    <?php echo e($consent->parent_id_number ?? 'N/R'); ?></p>
                                            </div>
                                        </div>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="space-y-1 text-sm">
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($consent->medical_allergies): ?>
                                                <p>🤧 Allergies: <?php echo e($consent->medical_allergies_detail ?: 'Oui'); ?></p>
                                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                            ...
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($consent->medical_cicatrisation): ?>
                                                <p>🩹 Cicatrisation difficile: Oui</p>
                                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($consent->medical_skin_disease): ?>
                                                <p>🩹 Maladie peau: <?php echo e($consent->medical_skin_disease_detail ?: 'Oui'); ?>
                                            ...
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($consent->medical_other): ?>
                                                <p>📝 Autres: <?php echo e($consent->medical_other); ?></p>
                                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                        </div>
                                    </div>
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 text-sm">
                                            <p><span class="text-ivoire-text/60">Prix:</span>
                                                <?php echo e($consent->total_price ? number_format($consent->total_price, 2, ',', ' ') . ' €' : 'N/R'); ?>

                                            </p>
                                            <p><span class="text-ivoire-text/60">Acompte:</span>
                                                <?php echo e($consent->deposit_amount ? number_format($consent->deposit_amount, 2, ',', ' ') . ' €' : 'N/R'); ?>
                                    ...
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <p class="text-sm">
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($consent->image_authorization === true): ?>
                                                <span class="text-vert-succes">✅ Accordée</span>
                                            <?php elseif($consent->image_authorization === false): ?>
                                                <span class="text-rouge-alerte">❌ Refusée</span>
                                            <?php else: ?>
                                                <span class="text-ambre-warning">⚠️ N/R</span>
                                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                        </p>
                                    </div>
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                    ...
                                    <div class="bg-noir-profond/50 rounded-lg p-3">
                                        ...
                                        <div class="space-y-1 text-sm">
                                            <p><span class="text-ivoire-text/60">Mention:</span>
                                                <?php echo e($consent->handwritten_mention ?? 'N/R'); ?></p>
...
<?php $component->withAttributes(['feature' => 'la traçabilité réglementaire']); ?>
                ...
                <div class="bg-gris-fonde text-ivoire-text rounded-xl p-4" x-data="{
                    ...
                    <button @click="showForm = !showForm" type="button"
                        ...
                        <svg class="w-5 h-5 transition-transform" :class="showForm ? 'rotate-180' : ''" fill="none"
                    ...
                    <div x-show="showForm" x-cloak x-collapse class="mt-4 pt-4 border-t border-titane/20">
                        <form action="<?php echo e(route($tattooer->routePrefix() . '.clients.traceability.store', $client)); ?>"
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($tattooer->isPiercer()): ?>
                                ...
                                <div class="flex items-center justify-between">
                                    <p class="text-xs font-bold text-ivoire-text/60 uppercase tracking-wider">Canules
                                        utilisées</p>
                                    <button type="button"
                                        @click="needles.push({ brand: '', lot_number: '', type: 'canule' })"
                                ...
                                <template x-for="(needle, ni) in needles" :key="'sn' + ni">
                                    <div class="bg-noir-profond/30 rounded-lg p-3 space-y-2">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-2">
                                                <span class="text-xs text-titane font-semibold"
                                                    x-text="'#' + (ni + 1)"></span>
                                                <select :name="'needles[' + ni + '][type]'" x-model="needle.type"
                                                    class="px-2 py-1 bg-noir-profond border border-titane/30 rounded text-ivoire-text text-xs focus:border-beige-peau">
                                                    <option value="canule">Canule</option>
                                                    <option value="aiguille_piercing">Aiguille piercing</option>
                                                </select>
                                            </div>
                                            <button type="button" @click="if(needles.length > 1) needles.splice(ni, 1)"
                                                x-show="needles.length > 1"
                                        ...
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            <input type="text" :name="'needles[' + ni + '][brand]'"
                                                x-model="needle.brand" placeholder="Marque (Caflon, Inverness...)"
                                            ...
                                            <input type="text" :name="'needles[' + ni + '][lot_number]'"
                                                x-model="needle.lot_number" placeholder="N° de lot"
                                                class="flex-1 px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                        </div>
                                    </div>
                                </template>
                                ...
                                <div class="flex items-center justify-between">
                                    <p class="text-xs font-bold text-ivoire-text/60 uppercase tracking-wider">Bijoux
                                        utilisés</p>
                                ...
                                <template x-for="(ink, ii) in inks" :key="'si' + ii">
                                    <div class="bg-noir-profond/30 rounded-lg p-3 space-y-2">
                                        <div class="flex items-center justify-between">
                                            <span class="text-xs text-titane font-semibold"
                                                x-text="'Bijou ' + (ii + 1)"></span>
                                            <button type="button" @click="if(inks.length > 1) inks.splice(ii, 1)"
                                                x-show="inks.length > 1"
                                                class="text-rouge-alerte/60 hover:text-rouge-alerte">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <input type="text" :name="'inks[' + ii + '][brand]'" x-model="ink.brand"
                                            placeholder="Marque / Type (Implant Grade Titane, Acier 316L...)"
                                            class="w-full px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            <input type="text" :name="'inks[' + ii + '][color]'" x-model="ink.color"
                                                placeholder="Taille / Calibre (ex: 1.2mm, 8mm)"
                                                class="flex-1 px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                            <input type="text" :name="'inks[' + ii + '][lot_number]'"
                            ...
                            <?php else: ?>
                                ...
                                <div class="flex items-center justify-between">
                                    <p class="text-xs font-bold text-ivoire-text/60 uppercase tracking-wider">Aiguilles &
                                        Cartouches</p>
                                    <button type="button"
                                        @click="needles.push({ brand: '', lot_number: '', type: 'aiguille' })"
                                        class="text-xs text-beige-peau hover:text-beige-peau/80 font-semibold">+
                                        Ajouter</button>
                                </div>
                                <template x-for="(needle, ni) in needles" :key="'sn' + ni">
                                    <div class="bg-noir-profond/30 rounded-lg p-3 space-y-2">
                                        ...
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            <input type="text" :name="'needles[' + ni + '][brand]'"
                                                x-model="needle.brand"
                                                placeholder="Marque (Cheyenne, FK Irons, Kwadron...)"
                                                class="flex-1 px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                            <input type="text" :name="'needles[' + ni + '][lot_number]'"
                                                x-model="needle.lot_number" placeholder="N° de lot"
                                                class="flex-1 px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                ...
                                <div class="flex items-center justify-between">
                                    <p class="text-xs font-bold text-ivoire-text/60 uppercase tracking-wider">Encres
                                        utilisées</p>
                                    <button type="button" @click="inks.push({ brand: '', color: '', lot_number: '' })"
                                        class="text-xs text-beige-peau hover:text-beige-peau/80 font-semibold">+
                                        Ajouter</button>
                                ...
                                <template x-for="(ink, ii) in inks" :key="'si' + ii">
                                    <div class="bg-noir-profond/30 rounded-lg p-3 space-y-2">
                                        ...
                                        <input type="text" :name="'inks[' + ii + '][brand]'" x-model="ink.brand"
                                            placeholder="Marque (World Famous, Eternal...)"
                                            class="w-full px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            <input type="text" :name="'inks[' + ii + '][color]'" x-model="ink.color"
                                                placeholder="Couleur"
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($standaloneTraces->count() > 0): ?>
                    <div class="bg-gris-fonde rounded-xl p-4 md:p-6">
                        <h3 class="text-sm font-bold text-ivoire-text/60 uppercase tracking-wider mb-3">📋 Traçabilités
                            manuelles</h3>
                        <div class="space-y-3">
                                    ...
                                    <div class="flex items-start justify-between mb-2">
                                        <div>
                                            <p class="font-semibold text-ivoire-text">
                                                <?php echo e($trace->tattoo_description ?? 'Tatouage'); ?></p>
                                            <p class="text-sm text-titane"><?php echo e($trace->body_zone ?? ''); ?> ·
                                                <?php echo e($trace->session_date?->format('d/m/Y') ?? ''); ?></p>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <span class="px-2 py-1 bg-vert-succes/20 text-vert-succes rounded text-xs">✅
                                                Validée</span>
                                            <?php echo $__env->make('partials.pdf-download-button', [
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $relevantAppointments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $apt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <div class="bg-gris-fonde rounded-xl p-4" x-data="{
                        ...
                        <div class="flex items-center justify-between cursor-pointer" @click="expanded = !expanded">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-titane">RDV
                                        <?php echo e($apt->start_datetime?->translatedFormat('l d F Y à H:i') ?? 'Date à définir'); ?>
                        ...
                        <div x-show="expanded" x-cloak x-collapse class="mt-4 pt-4 border-t border-titane/20">
                            <form action="<?php echo e(route($tattooer->routePrefix() . '.traceability.store', $apt)); ?>"
                                method="POST" enctype="multipart/form-data" class="space-y-4">
                                <?php echo csrf_field(); ?>

                                
                                <div class="flex items-center justify-between">
                                    <p class="text-xs font-bold text-ivoire-text/60 uppercase tracking-wider">
                                        Aiguilles &
                                        Cartouches</p>
                                    <button type="button"
                                        @click="needles.push({ brand: '', lot_number: '', type: 'aiguille' })"
                                ...
                                <template x-for="(needle, ni) in needles" :key="'an' + ni">
                                    <div class="bg-noir-profond/30 rounded-lg p-3 space-y-2">
                                        ...
                                        <div class="flex flex-col sm:flex-row gap-2">
                                            <input type="text" :name="'needles[' + ni + '][brand]'"
                                                x-model="needle.brand" placeholder="Marque (Cheyenne, FK Irons...)"
                                                class="flex-1 px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                            <input type="text" :name="'needles[' + ni + '][lot_number]'"
                                                x-model="needle.lot_number" placeholder="N° de lot"
                                                class="flex-1 px-3 py-2 bg-noir-profond border border-titane/30 rounded-lg text-ivoire-text placeholder-titane text-sm focus:border-beige-peau">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                    <div>
                                        <label class="text-xs text-titane block mb-1">Date
                                            stérilisation</label>
                                        <input type="date" name="sterilization_date"
                                    ...
                                    <div>
                                        <label class="text-xs text-titane block mb-1">N° lot
                                            stérilisation</label>
                                        <input type="text" name="sterilization_lot_number"
                                    ...
                                    <div>
                                        <label class="text-xs text-titane block mb-1">N° cycle
                                            autoclave</label>
                                        <input type="text" name="autoclave_cycle_number"
                                            ...
                                            <div class="flex gap-2 mt-2 flex-wrap">
                                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $lotPhotos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <div class="w-16 h-16 rounded-lg overflow-hidden bg-noir-profond cursor-pointer border border-titane/20 hover:border-beige-peau transition-colors"
                                                        ...
                                                        <img src="<?php echo e($photo->getUrl()); ?>"
                                                            alt="<?php echo e($photo->file_name); ?>"
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($trace && $trace->isComplete()): ?>
                                    <div class="mt-3">
                                        <?php echo $__env->make('partials.pdf-download-button', [
                                            'url' => route('pdf.traceability', $trace),
                                            'label' => '📄 Télécharger la traçabilité (PDF)',
                                            'size' => 'sm',
                                        ], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                                    </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
...
<?php $component->withAttributes(['feature' => 'la galerie médias client']); ?>
                ...
                <div class="bg-gris-fonde rounded-xl p-4 md:p-6">
                    ...
                    <form action="<?php echo e(route($tattooer->routePrefix() . '.clients.photos.upload', $client)); ?>"
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($clientPhotos->count() > 0): ?>
                    <div class="bg-gris-fonde rounded-xl p-4 md:p-6">
                        <h3 class="text-sm font-bold text-ivoire-text/60 uppercase tracking-wider mb-3">📸 Photos client
                            (<?php echo e($clientPhotos->count()); ?>)</h3>
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $clientPhotos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="aspect-square rounded-lg overflow-hidden bg-noir-profond relative group">
                                    <img src="<?php echo e($photo->getUrl()); ?>" alt=""
                                        ...
                                        onerror="this.style.display='none'">
                                    <form
                                        action="<?php echo e(route($tattooer->routePrefix() . '.clients.photos.delete', [$client, $photo->id])); ?>"
                                        method="POST"
                ...
                <div class="bg-gris-fonde rounded-xl p-4">
                    <h3 class="text-sm font-bold text-ivoire-text/60 uppercase tracking-wider mb-3">💬 Photos conversations
                        (<?php echo e($chatMedia->count()); ?>)</h3>
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $chatMedia; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $media): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="aspect-square rounded-lg overflow-hidden bg-noir-profond relative group">
                                    <img src="<?php echo e($media->getUrl()); ?>" alt=""
                                        ...
                                        onerror="this.style.display='none'">
                                    <form
                                        action="<?php echo e(route($tattooer->routePrefix() . '.client.media.delete', [$client, $media->id])); ?>"
                                        method="POST"
                                        class="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                        <button type="submit" onclick="return confirm('Supprimer ?')"
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $bookingRequests->filter(fn($br) => $br->deposit_paid_at); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $br): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="bg-gris-fonde rounded-xl p-4">
                        <h4 class="text-sm font-bold text-ivoire-text mb-3">📸 <?php echo e($br->tattoo_style ?? 'Tattoo'); ?> —
                            <?php echo e($br->body_zone ?? ''); ?> <span
                                class="text-xs text-titane font-normal ml-1">(<?php echo e($br->created_at->format('d/m/Y')); ?>)</span>
                        </h4>
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $tattooPhotos; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $photo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="aspect-square rounded-lg overflow-hidden bg-noir-profond relative group">
                                        <img src="<?php echo e($photo->getUrl()); ?>" alt=""
                                            ...
                                            onclick="window.openLightbox('<?php echo e($photo->getUrl()); ?>')">
                                        <form
                                            action="<?php echo e(route($tattooer->routePrefix() . '.client.media.delete', [$client, $photo->id])); ?>"
                                            method="POST"
                                            class="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                            <button type="submit" onclick="return confirm('Supprimer ?')"
                        ...
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        <form action="<?php echo e(route($tattooer->routePrefix() . '.client.photos.upload', [$client, $br])); ?>"
                            method="POST" enctype="multipart/form-data"
...
<?php $component->withAttributes(['feature' => 'les notes privées client']); ?>
                <div class="bg-gris-fonde rounded-xl p-4 md:p-6">
                    ...
                    <form action="<?php echo e(route($tattooer->routePrefix() . '.client.update-notes', $client)); ?>"
                        method="POST">
                        <?php echo csrf_field(); ?>
                        ...
                        <button type="submit"
                            class="mt-3 w-full sm:w-auto px-6 py-2 bg-beige-peau text-noir-profond rounded-lg font-semibold text-sm hover:bg-beige-peau/90 transition-colors">Enregistrer</button>
                    </form>
                </div>
             <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
    ...
    <script>
        ...
        function previewFiles(input) {
            var preview = input.closest('div').querySelector('.upload-preview') || input.parentElement.querySelector(
                '.upload-preview');
            if (!preview) return;
            preview.innerHTML = '';
