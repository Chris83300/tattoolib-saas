    ...
    $cSigned = $cBr
        ? \App\Models\Consent::where('booking_request_id', $cBr->id)
            ->whereNotNull('signed_at')
                                ...
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isConsentForm): ?>
                                    ...
                                    <div class="flex justify-center mb-4">
                                        <div
                                            ...
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($cSigned): ?>
                                                <div
                ...
                <div class="border-t border-titane/30 p-4">
                    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sendMessage', $conversation)): ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$bookingRequest->deposit_paid_at): ?>
                            ...
                            <?php elseif($conversation && $conversation->deposit_deadline_at): ?>
                                <?php
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($bookingRequest->status->value === 'accepted' && !$bookingRequest->deposit_paid_at): ?>
                <div class="mt-6 bg-titane/20 rounded-xl border border-titane/30 p-6 block md:block">
                    ...
                    <!-- Délai de paiement -->
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$bookingRequest->deposit_paid_at && $bookingRequest->deposit_deadline): ?>
                        <?php
                            $deadline = is_string($bookingRequest->deposit_deadline)
                                ? \Carbon\Carbon::parse($bookingRequest->deposit_deadline)
                                : $bookingRequest->deposit_deadline;
                        ...
                        <div class="mb-4 p-3 bg-jaune-alerte/10 border border-jaune-alerte/30 rounded-lg">
                            <p class="text-jaune-alerte text-sm">
                                ...
                                Délai de paiement : <?php echo e($deadline->format('d/m/Y à H:i')); ?>
                                ...
                                <?php elseif($daysRemaining <= 1): ?>
                                    <span class="block mt-1 text-rouge-alerte font-semibold">⚠️ Urgent - Dernier
                                        jour</span>
                                <?php else: ?>
                                    <span class="block mt-1">(<?php echo e($daysRemaining); ?> jour(s) restant(s))</span>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </p>
                        </div>
                    <?php elseif(!$bookingRequest->deposit_paid_at && $conversation && $conversation->deposit_deadline_at): ?>
                        <?php
                            $deadline = is_string($conversation->deposit_deadline_at)
                                ? \Carbon\Carbon::parse($conversation->deposit_deadline_at)
                                : $conversation->deposit_deadline_at;
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-vert-succes text-sm">
                                        Votre acompte de <?php echo e(number_format($bookingRequest->total_deposit_amount, 2)); ?>€ a
                                        bien été reçu.
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($bookingRequest->appointment_datetime): ?>
                                            ...
                                            <?php echo e($bookingRequest->appointment_datetime->format('d/m/Y à H:i')); ?>.
                                        <?php else: ?>
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($bookingRequest->price_range_min && $bookingRequest->price_range_max): ?>
                                <div class="flex justify-between">
                                    ...
                                    <span
                                        class="text-ivoire-text"><?php echo e(number_format($bookingRequest->price_range_min, 0)); ?>€
                                        -
            ...
            <div>
                <p class="font-semibold text-ivoire-text"><?php echo e($bookingRequest->bookable->user->name); ?></p>
        ...
        <div id="consent-modal" class="hidden fixed inset-0 z-50" style="background:rgba(0,0,0,0.85);"
            ...
            <div class="flex items-end sm:items-center justify-center min-h-screen p-0 sm:p-4">
                <div class="bg-gris-fonde w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[92vh] overflow-y-auto">
                    ...
                    <form action="<?php echo e(route('client.consent.store', $consentBr)); ?>" method="POST" class="p-4 space-y-4"
                        id="consent-form">
                        <?php echo csrf_field(); ?>
                        ...
                        <div class="bg-noir-profond/50 rounded-xl overflow-hidden" x-data="{ openRisk: null }">
                            ...
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = [['🦠', 'Infections', 'Malgré les précautions d\'hygiène strictes, un risque d\'infection bactérienne existe. Signes : rougeur qui s\'étend, gonflement, pus, fièvre. Consultez un médecin immédiatement.'], ['🤧', 'Réactions allergiques', 'Certaines encres (surtout rouges, jaunes, vertes) peuvent provoquer des réactions allergiques immédiates ou retardées. Signalez toute allergie connue ci-dessous.'], ['🩹', 'Cicatrisation', 'La cicatrisation prend 2 à 4 semaines avec des soins quotidiens. Une mauvaise cicatrisation peut altérer le résultat. Suivez les instructions de soin post-tatouage.'], ['🎨', 'Résultat esthétique', 'Le rendu final peut légèrement différer du dessin selon la texture de peau et la cicatrisation. Les couleurs peuvent paraître plus claires après cicatrisation.'], ['⚕️', 'Contre-indications', 'Tatouage déconseillé si : anticoagulants, grossesse/allaitement, diabète non contrôlé, immunodépression, problèmes de cicatrisation.']]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => [$icon, $title, $text]): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="border-t border-titane/10">
                                    <button type="button"
                                        @click="openRisk = openRisk === <?php echo e($i); ?> ? null : <?php echo e($i); ?>"
                                        class="w-full px-4 py-2.5 flex items-center justify-between text-left">
                                        <span
                                            class="flex items-center gap-2 text-sm text-ivoire-text/90"><?php echo e($icon); ?>

                                            <?php echo e($title); ?></span>
                                        <svg class="w-4 h-4 text-titane transition-transform"
                                            :class="{ 'rotate-180': openRisk === <?php echo e($i); ?> }" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>
                                    <div x-show="openRisk === <?php echo e($i); ?>" x-collapse>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = ['Diabète', 'Hémophilie', 'Épilepsie', 'Problèmes cardiaques', 'VIH/Hépatite', 'Troubles immunitaires']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cond): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <label
                                        class="flex items-center gap-2 text-xs text-ivoire-text/80 bg-noir-profond/30 rounded-lg px-3 py-2 cursor-pointer">
                                        <input type="checkbox" name="medical_conditions[]" value="<?php echo e($cond); ?>"
